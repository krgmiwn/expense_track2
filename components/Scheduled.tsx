...
const Scheduled: React.FC<ScheduledProps> = ({
  ...
  const getInitialForm = (): Omit<ScheduledTransaction, 'id'> => ({
    amount: 0,
    category: 'Other',
    type: 'EXPENSE',
    frequency: 'MONTHLY',
  ...
  const [form, setForm] = useState<Omit<ScheduledTransaction, 'id'>>(getInitialForm());
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onAdd(form);
    setIsAdding(false);
    setForm(getInitialForm()); // ✅ reset properly
  };
